  ...
  return (
    <main className="mx-auto w-full max-w-[1000px] px-6 pb-[45px]">
      <nav className="flex min-h-[66px] items-center border-b border-plum/[0.09]">
        <span className="text-[20px] font-extrabold tracking-[-1.2px]">
        ...
        <div className="ml-auto hidden gap-[7px] sm:flex">
          {STEPS.map((step) => (
            <span
              key={step.n}
              className={clsx("flex items-center text-[8px] font-extrabold", step.n <= 2 ? "text-mulberry" : "text-porcelain/50")}
            >
              <b
                className={clsx(
      ...
      <section className="mx-auto my-[43px] max-w-[510px]">
        ...
        <h1 className="my-[10px] font-display text-[35px] leading-[1.1]">
          Create a new password
        </h1>
        <p className="mb-[18px] text-[10px] leading-[1.55] text-porcelain/60">
          First confirm the code sent to <b className="text-champagne">{maskedEmail}</b>, then choose a
          new password.
        </p>

        <form onSubmit={handleSubmit} className="rounded-[22px] border border-plum/[0.1] bg-white p-[22px] text-ink shadow-[0_8px_23px_rgba(13,6,14,0.07)]">
          ...
          <button
            ...
            className="mt-[14px] w-full rounded-pill bg-plum px-3 py-[11px] text-[9px] font-extrabold text-[#F6F0E8] disabled:opacity-50"
          >
